...
class MaskingService:
    ...
    def apply_pdf_masks(
        ...
        "Entity Label" / "entity_label"
            ...
            label of the form ``{EntityType}_{L}`` (e.g. ``Full_Name_A``) where
            ``L`` is a per-type letter (A–Z) then number for >26 occurrences.
            Identical ``original_text`` values receive the same label.
            If ``replacement_text`` is already set (pre-generated by the
            document intelligence model), that value is used directly.
